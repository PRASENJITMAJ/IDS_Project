...
class PreprocessingGUI:
    ...
    def create_gui(self):
        ...
        ttk.Button(process_frame, text="5. Apply PCA", command=self.apply_pca).grid(row=0, column=4, padx=5)
    ...
    def apply_pca(self):
        if self.processed_df is None:
            ...
            return
        try:
            self.processed_df = self.preprocessing.apply_pca(self.processed_df)
            self.status_var.set("Applied PCA")
            self.update_preview()
            self.visualize_data()
        except Exception as e:
            messagebox.showerror("Error", f"Error applying PCA: {str(e)}")
